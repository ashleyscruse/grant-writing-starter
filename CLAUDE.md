...
- `.claude/skills/` — reusable `SKILL.md` instructions, one per task. (Hidden machinery the researcher never needs to open; you load it automatically.)
  ...
  - `reviewer-builder/` — run the review panel on a draft, synthesize the reads, save `proposal/reviews/vN-review.md`, then check compliance and formatting. A separate job from writing.
  - `reviewer-skeptic/`, `reviewer-generalist/`, `reviewer-subject-expert/`, `reviewer-program-officer/` — the four panel personas `reviewer-builder` runs; each grounds itself in the solicitation and reviews in its lens.
