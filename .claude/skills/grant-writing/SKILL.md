...
- **Budget:** the numbers live in `proposal/budget.xlsx` (a spreadsheet the researcher edits in Excel or Google Sheets). Read it directly and draft `proposal/budget-justification.tex` from it, explaining every line. Keep the two in sync: if the sheet changes, update the justification, and make sure every narrative activity has a budget line and vice versa.
